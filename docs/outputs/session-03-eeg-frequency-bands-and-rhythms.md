...
EEG 신호를 주파수 대역별로 정리하고, 각 대역을 BCI feature로 해석할 때 필요한 기준과 한계를 구분한다.

이번 문서의 핵심은 단순히 delta, theta, alpha, beta, gamma의 의미를 외우는 것이 아니라, 다음을 구분하는 것이다.
...
| --- | --- | --- | --- |
| EEG observation | waveform, amplitude, frequency, distribution, phase, synchrony, reactivity | EEG는 단일 주파수 값이 아니라 여러 특성이 결합된 패턴이다 | feature 해석 시 단일 값보다 조건별 변화와 반복성이 중요하다 |
| Signal unit | wave, rhythm, background activity | 주파수 대역과 리듬은 같은 개념이 아니다 | alpha band power와 alpha rhythm을 구분해야 한다 |
| Normal adult EEG | posterior alpha, low-amplitude beta, symmetry | 정상 성인 EEG는 상태와 조건에 따라 달라진다 | baseline과 condition을 구분해야 한다 |
| Reactivity | alpha blocking, task-related change | 자극이나 행동에 따라 EEG가 변하는지가 중요하다 | BCI feature는 상태 변화에 반응해야 한다 |
| Frequency bands | delta, theta, alpha, SMR, beta, gamma | 각 대역은 해석 가능성이 다르지만 단독으로 심리 상태를 증명하지는 않는다 | feature 후보를 선정할 때 과도한 해석을 피해야 한다 |
| Artifact | eye blink, eye movement, EMG, movement, line noise | 뇌 신호가 아닌 전기적·생리적 신호가 EEG에 섞인다 | preprocessing 없이 feature를 해석하면 잘못된 결론을 낼 수 있다 |
...
→ EEG feature는 단일 값이 아니라 조건에 따라 변하는 pattern으로 봐야 한다.
...
예를 들어 alpha band와 alpha rhythm은 같지 않다.
...
→ 초기 EEG 분석에서는 alpha reactivity를 먼저 확인하는 것이 합리적이다.
...
따라서 현재 단계에서는 단일 17 Hz peak보다 다음과 같이 band power로 정의하는 것이 더 적절하다.
...
→ Low beta는 focus 후보 feature로 남기되, 단일 주파수가 아니라 band-level feature로 다루는 것이 적절하다.
...
→ Gamma/40 Hz는 장기적으로 검토할 수 있는 연구 후보로 남기되, 초기 feature 선정에서는 alpha와 low beta보다 낮은 우선순위로 둔다.

### 6. Artifact 구분은 feature extraction 이전에 필요하다
...
→ Feature를 보기 전에 raw signal과 artifact를 먼저 확인해야 한다.
...
| Level | What can be said | What should not be said |
| --- | --- | --- |
| Frequency band | 특정 주파수 범위의 power가 증가/감소했다 | 특정 심리 상태가 직접 측정되었다 |
| Rhythm | 위치, 형태, 반응성을 가진 반복 패턴이 관찰되었다 | 주파수만으로 rhythm이라고 단정한다 |
| Alpha suppression | 조건 변화에 따라 alpha power가 감소했다 | 피험자가 반드시 집중했다 |
| Low beta increase | 과제 조건에서 low beta power가 증가했다 | 문제 해결 능력이 향상되었다 |
| Gamma activity | high-frequency activity가 관찰되었다 | 고차 인지나 peak performance가 증명되었다 |
| BCI feature | 조건별 feature 차이를 이용할 수 있다 | 뇌의 의도 자체를 직접 읽었다 |
